...
# Ask for endpoint
$optApi = Read-Host "Enter POST URL directly or type 'file' to browse"
$ApiUrl = if ($optApi -eq 'file') { Get-Content (Select-File) } else { $optApi }

# Ask for payload
$optPayload = Read-Host "Enter payload directly (JSON) or type 'file' to browse"
$MyData = if ($optPayload -eq 'file') { 
    Get-Content (Select-File) | ConvertFrom-Json 
} else { 
