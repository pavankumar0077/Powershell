...
# Flatten nested JSON with clean column names
...
# Clean column names by removing 'data.' prefix
function Clean-ColumnNames {
    param($obj)
    $cleaned = @{}
    foreach ($p in $obj.PSObject.Properties) {
        $cleanName = if ($p.Name.StartsWith("data.")) { 
            $p.Name.Substring(5) 
        } else { 
            $p.Name 
        }
        $cleaned[$cleanName] = $p.Value
    }
...
# Flatten response and clean column names
Write-Host "2. Flattening response and cleaning column names..." -ForegroundColor Yellow
$flattened = Flatten-Json $resp
$row = Clean-ColumnNames $flattened
